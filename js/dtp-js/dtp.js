require([
  ...
  function(
    ...
    var dtpUrl, map, view, locateBtn, resultsLayer, dtpExpand,
    ...
    dtpUrl = "http://infopublic.pravstat.kz:8399/arcgis/rest/services/rta/MapServer/0";
    ...
    resultsLayer = new GraphicsLayer({
      visible: true,
      title: "Аварийность г.Акколь"
    });

    view.ui.add("topLeft", "top-left");

    // Аварийность
    dtpExpand = new Expand({
      ...
      content:
        "<div class='form-style-10' id='optionsDiv'>"+
          ...
          "<form>"+
            ...
            "</div>"+
            "<div class='section'><span>2</span>Выберите категорию аварийности</div>"+
            "<div class='inner-wrap'>"+
              "<label>Категория ДТП"+
                "<select name='hardId' id='hardId'>"+
                  "<option value=''>Все</option>"+
                  "<option value='0'>Автомобильная авария</option>"+
                  "<option value='1'>Столкновение с пешеходом</option>"+
                  "<option value='2'>Другое ДТП</option>"+
                "</select>"+
              "</label>"+
            "</div>"+
    ...
    view.ui.add(dtpExpand, "bottom-right");
    ...
    var layerList = new LayerList({
      ...
      listItemCreatedFunction: function(event) {
        ...
        if(item.layer.title == "Векторный слой г.Акколь" || item.layer.title == "Аэро-фото снимок г.Акколь"
          || item.layer.title == "Аварийность г.Акколь"){
          if(item.layer.title == "Векторный слой г.Акколь" || item.layer.title == "Аварийность г.Акколь"){
            item.panel = {
              content: "legend",
              open: false
            };
    ...
    view.when(function() {
      ...
      function doQuery(){
        ...
        var qTask, query, sel_month, sel_yr, opt, hard_opt, sqlTxt;
        qTask = new QueryTask({
          url: dtpUrl
        });

        query = new Query({
          outFields: ["RTA_TYPE,RTA_DATE,FD1ID,YR,PERIOD,FD1R05P1,FD1R01P1,FD1R07P2,FD1R07P3,FD1R07P4,FD1R071P1,FD1R09P1,"+
          "FD1R14P1,FD1R141P1,FD1R13P1,FD1R13P2,FD1R02P1K,FD1R06P1,FD1R06P2,FD1R06P3,VEHICLE_CATEGORY,IS_PUBLIC_TRANSPORT,FD1R09P1_ID"],
        ...
        if (document.getElementById("scales").checked == true){
          sqlTxt = "(PERIOD>='01' AND PERIOD<='" + sel_month + "') AND YR=" + sel_yr;
        }else{
         sqlTxt = "PERIOD=" + sel_month + " AND YR=" + sel_yr;
        }
        if (hard_opt != ""){
            sqlTxt = sqlTxt + " AND RTA_TYPE=" + hard_opt;
        }

        resultsLayer.removeAll();
        printResults.innerHTML = "";
		    printResults.style.display = "none";
        printResults.color = "white";
        query.where = sqlTxt;
        ...
        function getResults(response){
          var peakResults = response.features.map(function(point) {
            point.symbol = setSymbolDTP(point.attributes.RTA_TYPE);
            point.attributes.RTA_TYPE = changeContentDTP(point.attributes.RTA_TYPE);
            point.attributes.RTA_DATE = MyDate(new Date(point.attributes.RTA_DATE));
      ...
      popupTemplate = {
        ...
        content: [{
          ...
          fieldInfos: [{
            ...
            fieldName: "FD1R05P1",
            label: "Время суток совершения ДТП",
            format: {
              places: 0,
              digitSeperator: true
            }
            ...
            fieldName: "FD1R07P3",
            label: "Наружное освещение в темное время ",
            format: {
              places: 0,
              digitSeperator: true
            }
            ...
            fieldName: "FD1R071P1",
            label: "Зафиксированные недостатки улично-дорожной сети",
            format: {
              places: 0,
              digitSeperator: true
            }
